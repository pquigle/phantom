...
!   - rota_ratio     : rotation or direct out
...
!
 ...
 real         :: rota_ratio    = 0.0      ! azimuthal velocity as a fraction of Keplerian velocity
 !real         :: rinject       = 1.0      

 integer      :: shuffle_count = 0 
...
subroutine inject_particles(time,dtlast,xyzh,vxyzu,xyzmh_ptmass,vxyz_ptmass,&
 ...
 use io,            only:fatal
 use part,          only:nptmass,massoftype,igas,hfact,ihsoft,shuffle_part,isdead_or_accreted,kill_particle
 use partinject,    only:add_or_update_particle
 use physcon,       only:twopi,gg,kboltz,mass_proton_cgs
 use random,        only:get_random_pos_on_sphere, get_gaussian_pos_on_sphere, gauss_random
 ...
 if (shuffle_count > 1000) then 
  write(*,*) "time is ", time, "dtlast is", dtlast
  do i=1,npart
 ...
 do i=1,npinject
    ...
    case (1)
      ...
      r_kep = sqrt(dx(1)**2+dx(2)**2) * rinject
      v_kep = sqrt(m2/r_kep)
      v_phi = v_kep*rota_ratio
      call rotatevec(dx, rotaxis, theta_rad)
      
      vhat = dx
      call cross_product3D(veczprime, dx, vhat)

      vxyz      = v_r*dx + v_phi*vhat + v2 
      xyz       = r2 + rinject*dx
    case default
...
subroutine write_options_inject(iunit)
 ...
 if (random_type==1) then
    ...
    call write_inopt(rota_ratio, 'rota_ratio', 'azimuthal velocity as a fraction of Keplerian velocity', iunit)
 endif
 if (wind_type==1) then
...
subroutine read_options_inject(name,valstring,imatch,igotall,ierr)
 ...
 case('wind_speed')
    ...
    ngot = ngot + 1
 case('rota_ratio')
    read(valstring,*,iostat=ierr) rota_ratio
    ngot = ngot + 1
 case('r_inject')
